...
public class Sudoku : MonoBehaviour, IPointerMoveHandler, IPointerDownHandler
{
    ...
    [SerializeField] private TextMeshProUGUI problemRatingText;
    ...
    private void ResetGame()
    {
        ...
        int[,] level = SudokuProblemDatas.Instance.GetRandomProblem(Stats.SudokuRating);
    ...
    private void LoadLevel(int[,] level)
    {
        for (int j = 0; j < boxes.GetLength(0); j++) {
            for (int i = 0; i < boxes.GetLength(1); i++) {
                if (level[i, j] != 0) {
                    boxes[j, i].SetAsFixed(level[i,j]);
                    continue;
                }
